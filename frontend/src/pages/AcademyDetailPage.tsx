...
type Tab = 'schedule' | 'members' | 'classes' | 'settings' | 'graduations' | 'attendances'
...
export default function AcademyDetailPage() {
  ...
  const tabLabel = (t: Tab): string => {
    switch (t) {
      ...
      case 'attendances': return translate('academy.tab.attendances')
    }
  }
    ...
    <div className="mx-auto w-full max-w-3xl space-y-6 overflow-x-hidden pb-20">
      ...
      {academy && id && (
        <div className="space-y-6">
          <div className="rounded-xl border border-[var(--border-card)] bg-[var(--bg-card)] p-5 shadow-sm sm:p-6">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-start">
              <div className="flex min-w-0 flex-1 items-start gap-4">
                <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-[var(--bg-subtle)] text-[var(--text-muted)] sm:h-14 sm:w-14">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  {academy.address && (
                    <div className="mt-1 flex items-start gap-1.5 text-sm text-[var(--text-muted)]">
              ...
              <div className="flex shrink-0 flex-row flex-wrap items-center gap-2.5 sm:h-14 sm:flex-col sm:items-end sm:justify-center">
                {membershipLoading ? (
                  <span className="text-xs text-[var(--text-subtle)]">
                    <LoaderIcon size={14} className="inline" />
                  </span>
                ) : isMember ? (
                  <span className="inline-flex w-fit items-center gap-1.5 rounded-full bg-emerald-500/10 px-3 py-1 text-xs font-medium text-emerald-400">
                    {translate('academy.alreadyMember')}
                  </span>
                ) : isPending ? (
                  <div className="flex flex-wrap items-center gap-2">
                    <span className="inline-flex w-fit items-center gap-1.5 rounded-full bg-amber-500/10 px-3 py-1 text-xs font-medium text-amber-400">
                      <HourglassIcon size={14} />
                      {translate('academy.pendingApproval')}
                    </span>
                    <button
                      onClick={() => refetchMembership()}
                      disabled={membershipFetching}
                      className="inline-flex w-fit shrink-0 items-center gap-1.5 rounded-lg border border-[var(--border-card)] bg-[var(--bg-page)] px-2.5 py-1 text-xs text-[var(--text-muted)] hover:text-[var(--text-primary)] disabled:opacity-50"
                    >
                      <RefreshIcon size={13} className={membershipFetching ? 'animate-spin' : undefined} />
                      {translate('academy.checkStatus')}
                    </button>
                  </div>
                ) : (
                  <button
                    ...
                    className="w-full rounded-lg bg-[var(--text-primary)] px-4 py-2.5 text-sm font-medium text-[var(--bg-page)] hover:opacity-90 disabled:opacity-50 sm:w-auto sm:py-2"
                  >
                ...
                {isMember && !isOwner && (
                  <button
                    ...
                    className="inline-flex w-fit shrink-0 items-center gap-1 rounded-lg border border-rose-500/20 bg-rose-500/10 px-3 py-1 text-xs text-rose-400 hover:bg-rose-500/20 disabled:opacity-50"
                  >
          ...
          {isMember && availableTabs.length > 1 && (
            <div className="no-scrollbar flex gap-1 overflow-x-auto border-b border-[var(--border-header)] sm:gap-2">
              {availableTabs.map((t) => (
                <button
                  key={t}
                  onClick={() => setTab(t)}
          ...
          {isMember && activeTab === 'attendances' && (
            <AttendanceHistorySection variant="member" academyId={id} userId={profile?.id} />
          )}
        </div>
      )}
    </div>
